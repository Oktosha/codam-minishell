...
/*   Updated: 2023/08/14 12:02:42 by codespace     ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	l_tk_symbol_error(t_tk_so_far *so_far)
{
	...
	len = mini_strlen(so_far->token.data);
	if (len > (so_far->token.length + 4))
		mini_write(2, so_far->token.data, so_far->token.length + 4);
	else
		mini_write(2, so_far->token.data, len);
